...
class SolverLamda:
    def __init__(self, name: str = "PAC_CBF", num_vars: int = 0, coe_lb=0, coe_ub=1e3, verbose: int = 0):
        self.solver_model = gp.Model(name)
        self.solver_model.setParam('OutputFlag', verbose)

        self.coe = self.solver_model.addMVar(shape=(num_vars,), lb=coe_lb,
    ...
    def add_constraint(self, cons, constant):
        # add upper bound of var( coe_ub and 1 )
        cons_coe_upper_bound = np.ones(cons.shape[1])
        cons_coe_upper_bound = np.diag(cons_coe_upper_bound)

        cons = np.append(cons, cons_coe_upper_bound, axis=0)

        constant_upper = self.coe_ub * np.ones(cons.shape[1])
        constant_upper[-1] = 1

        constant = np.concatenate((constant, constant_upper))

        # add lower bound of var( 0 )
        # cons_coe_lower_bound = -1 * np.ones(cons.shape[1] - 1)
        # cons_coe_lower_bound = np.diag(cons_coe_lower_bound)
        #
        # new_col_lower_bound = np.zeros((cons_coe_lower_bound.shape[0], 1))
        # cons_coe_lower_bound = np.hstack((cons_coe_lower_bound, new_col_lower_bound))
        # cons = np.append(cons, cons_coe_lower_bound, axis=0)
        #
        # constant_lower = np.zeros(cons.shape[1] - 1)
        # constant = np.concatenate((constant, constant_lower))
